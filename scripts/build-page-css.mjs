...
// --name=<slug> scopes both the input picker (looks for `<slug>-react-page-*.json`)
// and the generated output filenames (writes `<slug>.css`, `<slug>.tree.json`,
// `<slug>.stylesheets.json` instead of the default `page.*`). Default slug is
// `page` so the existing pipeline continues to work unchanged.
...
const inputPrefix = nameSlug ? `${nameSlug}-react-page-` : 'react-page-'

async function pickLatest() {
  const entries = await readdir(SCRAPPED)
  const matches = entries
    .filter((f) => f.startsWith(inputPrefix) && f.endsWith('.json'))
    .sort()
  if (matches.length === 0) {
    console.error(`No ${inputPrefix}*.json found in scrapped/. Run \`npm run extract:page${nameSlug ? ` -- … --name=${nameSlug}` : ''}\` first.`)
    process.exit(1)
  }
...
log(`Output : <generated>/${outName}.{css,tree.json,stylesheets.json}`)
...
const cssPath = resolve(GEN_DIR, `${outName}.css`)
